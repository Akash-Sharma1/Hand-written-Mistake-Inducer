...
def process_G(s, idx, last, slavogermanic):
    if s.get(idx + 1) == 'H':
        if idx > 0 and not isvowel(s[idx - 1]):
            return ('K',  'K', 2)
        elif idx == 0:
            if s.get(idx + 2) == 'I':
                return ('J',  'J', 2)
        ...
        if idx == 1 and isvowel(s[0]) and not slavogermanic:
            return ('KN',  'K', 2)
...
def process_H(s, idx, last, slavogermanic):
    if ((idx == 0 or isvowel(s.get(idx-1))) and
       isvowel(s.get(idx + 1))):
        return ('H', 'H', 2)
    else:
        return ('', '', 1)
    ...
    if s.get(idx - 1, 3) in {'ISL', 'YSL'}:
        ...
        if s.get(idx + 2) == 'H':
            if s.get(idx + 3, 2) in {'OO', 'ER', 'EN', 'UY', 'ED', 'EM'}:
                ...
                if (idx == 0 and not isvowel(s.get(3)) and s.get(3) != 'W'):
                    return ('X', 'S', 3)
                else:
                    return ('X', 'X', 3)
...
def process_W(s, idx, last, slavogermanic):
    if s.get(idx, 2) == 'WR':
        return ('R', 'R', 2)
    elif ((idx == last and isvowel(s.get(idx - 1))) or
        ...
        return ('TS', 'FX', 4)
          ...
          (isvowel(s.get(idx + 1)) or s.get(idx, 2) == 'WH')):
        if isvowel(s.get(idx + 1)):
            return ('A', 'F', 1)
        else:
            return ('A', 'A', 1)
    else:
